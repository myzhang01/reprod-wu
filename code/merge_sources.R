...
raw_source1 = read.csv(paste0(dir_data,"gender_sample_by_classifiers/EJR_gender_dataset_Jan2018.csv"),
...
history = read.csv(paste0(dir_data, "NBER/author-history-merged.csv"),
                   stringsAsFactors = FALSE)
...
full_sample = merge(raw_source1,
                    raw_source2,
                    by = c("title_id1", "post1"),
                    all = T) # [1] 1742657       5
...
EJR_raw = read.csv(paste0(dir_data, "raw/EJR0_raw_text_cleaned.csv"),
                   stringsAsFactors = FALSE)
...
full_sample = merge(full_sample,
                    EJR_raw[, c("title_id1", "post1", "raw_cat", "poster_name", "poster_id")],
                    by = c("title_id1", "post1"),
                    all.x = T)
...
# Load main stats
main_stats = read.csv(paste0(dir_data, "raw/main_stats_cleaned.csv"),
                      stringsAsFactors = FALSE)
...
# duplicate "titles" in main_stats: keep the latest ones (occurring FIRST in this data as it started from Page 1)
...
initial_time$time_first_str = sapply(initial_time$raw,
                                     function(x) {
                                       substr(x, gregexpr("Posted on:", x)[[1]] + 10, nchar(x))
                                     })
...
get_yr <- function(x) {
  if (length(grep("year", x)) == 0) {
    0
  } else {
    as.numeric(substr(x, 1, 2))
  }
}
full_sample$yr_latest = sapply(full_sample$time_latest_str, function(x) {
  if (!is.na(x)) {
    get_yr(x)
  } else{
    NA
...
full_sample$yr_first = sapply(full_sample$time_first_str, function(x) {
  if (!is.na(x)) {
    get_yr(x)
  } else{
    NA
...
get_month <- function(x)
  if (length(grep("month", x)) == 0) {
    NA
  } else{
    as.numeric(substr(x, 1, gregexpr("month", x)[[1]] - 1))
  }
...
full_sample[is.na(full_sample$month_first), "month_first"] = sapply(full_sample[is.na(full_sample$month_first), "time_first_str"],
                                                                    function(x)
                                                                      if (gregexpr("year", x) == -1 &
                                                                          !is.na(x)) {
                                                                        0
                                                                      } else{
                                                                        NA
                                                                      }) # within 1 month: days, weeks
full_sample[full_sample$month_first == 12 &
              !is.na(full_sample$month_first), "month_first"] = 11
...
full_sample[is.na(full_sample$month_latest), "month_latest"] = sapply(full_sample[is.na(full_sample$month_latest), "time_latest_str"],
                                                                      function(x)
                                                                        if (gregexpr("year", x) == -1 & !is.na(x)) {
                                                                          0
                                                                        } else{
                                                                          NA
                                                                        })
full_sample[full_sample$month_latest == 12 &
              !is.na(full_sample$month_latest), "month_latest"] = 11
...
write.csv(full_sample,
          paste0(dir_data, "full_sample2019.csv"),
          row.names = FALSE)
