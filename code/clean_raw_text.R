...
EJR0[EJR0$post1 == 1, "raw_cat"] = gsub("<url>https://www.econjobrumors.com/topic/", 
                                        "", 
                                        EJR0[EJR0$post1 == 1, "raw_cat"])
EJR0[EJR0$post1 == 1, "raw_cat"] = gsub("</url>", 
                                        "", 
                                        EJR0[EJR0$post1 ==  1, "raw_cat"])
EJR0[EJR0$post1 == 1, "raw_cat"] = gsub("-",
                                        " ", 
                                        EJR0[EJR0$post1 == 1, "raw_cat"])
...
EJR0$n_YH0 = sapply(EJR0$raw_cat, function(x) {
  length(gregexpr('-YH0-', x)[[1]])
}) 
...
get_name <- function(x) {
  if (length(gregexpr('-YH0-', x)[[1]]) >= 2) {
    substr(x, gregexpr('-YH0-', x)[[1]][1] + 5, gregexpr('-YH0-', x)[[1]][2] -
             1)
  } else{
    ""
  }
...
get_id <- function(x) {
  if (length(gregexpr('-YH0-', x)[[1]]) == 2) {
    ...
    substr(x, gregexpr('-YH0-', x)[[1]][2] + 5, gregexpr('-YH0-', x)[[1]][3] -
             1)
  } else{
    ""
  }
...
get_post <- function(x) {
  if (length(gregexpr('-YH0-', x)[[1]]) == 1) {
    x
  } else if ((length(gregexpr('-YH0-', x)[[1]]) == 2)) {
    ""
  } else{
...
# REVISION: 'str_only_Nov2017.csv' not in reproduction package; comment out
