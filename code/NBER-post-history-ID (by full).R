...
EJR = read.csv(paste0(dir_data, "raw/EJR0_raw_text_cleaned.csv"),
               stringsAsFactors = FALSE)
EJR = EJR[EJR$post_miss == 0, c("title_id1", "post1", "raw_cat")]
dim(EJR) # [1] 2172129       3; still 223,475 threads

# (2) authors
authors = read.csv(paste0(dir_data, "NBER/nber-author-info.csv"),
                   stringsAsFactors = FALSE)
authors = authors[, c("full_name", "first_name", "last_name")]
sum(duplicated(authors$full_name)) # 0

# special characters ("//*" in 2 names)
authors[authors$full_name == "FRANCESCO DAC**TO", c("full_name", "last_name")] =
  c("FRANCESCO DACUNTO", "DACUNTO")
authors[authors$full_name == "IS**TA RAJANI", c("full_name", "first_name")] =
  c("ISHITA RAJANI", "ISHITA")
...
for (i in 1:nrow(authors)) {
  ...
  print(c(i, name))
  EJR$by_full = sapply(EJR$raw_cat, function(x) {
    length(grep(name, tolower(x)))
  }) # 1 if matched, 0 otherwise (length(integer(0))=0)
  if (sum(EJR$by_full) != 0) {
    ...
    write.table(
      history,
      paste0(dir_data, "NBER/author-history-ID-by-full.csv"),
      append = TRUE,
      col.names = FALSE,
      sep = ",",
      row.names = FALSE
